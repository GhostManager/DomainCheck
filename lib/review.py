...
class DomainReview(object):
    ...
    def check_domain_status(self, domains_list, filter_list=None):
        ...
        with click.progressbar(domains_list, 
            ...
            for item in bar:
                ...
                dns_records = []
                namecheap_records = self.get_domain_dns(domain)
                ...
                lab_results[domain]['expiration'] = expired
    ...
    def output_csv(self, lab_results):
        ...
        csv_headers = ["Domain", "Domain Health", "DNS Health", "DNS", "Whois Privacy", "Expired", 
        ...
        with open('domain_health.csv', mode='w') as report:
            ...
            for domain in lab_results:
                ...
                expired = lab_results[domain]['expiration']
                ...
                report_writer.writerow([domain, health, health_dns, ", ".join(dns_records), whois, expired, 
                                        ", ".join(parsed_cats), ", ".join(xforce), 
                                        ", ".join(talos), ", ".join(bluecoat), 
    ...
    def print_confluence_table(self, lab_results):
        ...
        |  = Separates cells
        """
        table_headers = "||Domain||Domain Health||DNS Health||Whois Privacy||Expired||All Categories||IBM X-Force||Talos||Bluecoat||Fortiguard||OpenDNS||Trend Micro||MX Toolbox"
        ...
        for domain in lab_results:
            ...
            expired = lab_results[domain]['expiration']
            if expired == "False":
                expired = self.green_text + expired + self.color_end
            else:
                expired = self.red_text + expired + self.color_end
            ...
            click.secho('|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|'.format(
                        domain, health, health_dns, ", ".join(dns_records), whois, expired, 
                        ", ".join(parsed_cats), ", ".join(xforce), 
                        ", ".join(talos), ", ".join(bluecoat), 
                        ", ".join(fortiguard), ", ".join(opendns), 
                        ", ".join(trendmicro), ", ".join(mxtoolbox)), 
